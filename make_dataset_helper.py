...
def prepare_mnist():
    X_train =  loadmat("./mat_files/MNIST.mat")["trainX"].astype(np.float32)
    X_test =  loadmat("./mat_files/MNIST.mat")["testX"].astype(np.float32)
    T_train =  loadmat("./mat_files/MNIST.mat")["trainY"].astype(np.float32)
    T_test=  loadmat("./mat_files/MNIST.mat")["testY"].astype(np.float32)
    return X_train, X_test, T_train, T_test
...
def prepare_glass():
    train_num = 174
    test_num = 40
    X = loadmat("./mat_files/Glass.mat")["featureMat"].astype(np.float32)
    T = loadmat("./mat_files/Glass.mat")["labelMat"].astype(np.float32)
...
def prepare_wine():
    train_num = 140
    test_num = 38
    X = loadmat("./mat_files/Wine.mat")["featureMat"].astype(np.float32)
    T = loadmat("./mat_files/Wine.mat")["labelMat"].astype(np.float32)
    # X = ((X.T - X.mean(axis=1)) / X.std(axis=1)).T
    random_lists = np.random.choice(range(X.shape[1]), train_num + test_num, replace=False)
    random_train_lists = random_lists[:train_num]
    random_test_lists = random_lists[train_num:]
...
def prepare_iris():
    train_num = 120
    test_num = 30
    X = loadmat("./mat_files/Iris.mat")["featureMat"].astype(np.float32)
    T = loadmat("./mat_files/Iris.mat")["labelMat"].astype(np.float32)
    # X = ((X.T - X.mean(axis=1)) / X.std(axis=1)).T
    random_lists = np.random.choice(range(X.shape[1]), train_num + test_num, replace=False)
    random_train_lists = random_lists[:train_num]
    random_test_lists = random_lists[train_num:]
